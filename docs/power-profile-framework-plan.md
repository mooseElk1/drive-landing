...
## Power Measurement & Calibration Strategy

### The Two Channels

The app derives power from two sources that describe the same physical output through different measurement approaches:

- **Derived power** (`power_magnitude`) — computed from `P = m × a × v` using IMU data. Always available. Does not account for surface friction.
- **Friction-corrected power** (`peakPowerTrue`) — derived power adjusted for the surface friction coefficient (μ). Requires a validated friction detection pipeline. Not yet available.

Rather than a binary switch between these channels, the app treats them as a **calibration pair**: every sprint where both are available is a data point that teaches the system their relationship. Over time, derived power becomes increasingly accurate even when friction detection is unavailable.

---

### Four-Tier `PowerMeasurementMode`

Replaces the previous binary `PowerSource`. Every sprint is stamped with the mode that produced its power value:

| Mode                   | Description                                                     | When active                                                                 |
| ---------------------- | --------------------------------------------------------------- | --------------------------------------------------------------------------- |
| `'raw'`                | Derived power only, no correction                               | Friction detection unavailable, calibration threshold not met               |
| `'calibrated'`         | Derived power × learned surface ratio                           | Friction detection unavailable, but ≥5 calibration sprints for this surface |
| `'friction_corrected'` | Live friction detection applied                                 | Friction detection active and Medium/High confidence                        |
| `'fused'`              | Friction-corrected cross-validated against calibration estimate | Both channels active and agree within 5%                                    |

Stored on every `SprintPowerRecord` as `powerMeasurementMode: PowerMeasurementMode`.

---

### `power-source-service.ts` — Tiered Selection

At sprint-save time, selects the highest available mode:

```ts
function resolveSprintPower(
  metrics: WorkoutMetrics,
  surface: SurfaceType,
  calibration: CalibrationStats | null
): { power: number; mode: PowerMeasurementMode };
```

**Hierarchy (highest available wins):**

1. **Fused** — friction Medium/High confidence AND calibration ≥ threshold AND estimates agree within 5%
2. **Friction corrected** — friction Medium/High confidence but calibration insufficient or estimates diverge
3. **Calibrated** — friction unavailable/Low AND `calibration.sampleCount >= MIN_CALIBRATION_SAMPLES` (default: 5)
4. **Raw** — none of the above

---

### `calibration-service.ts` — Learning the Ratio

When both channels are available for a sprint:

```ts
calibrationRatio = frictionCorrectedPower / derivedPower;
```

Stored on `SprintPowerRecord` as `calibrationRatio: number | null`. The service maintains running statistics per surface type using Welford's online algorithm (no raw history retained):

```ts
type CalibrationStats = {
  meanRatio: number;
  variance: number; // low variance = stable, trustworthy ratio
  sampleCount: number;
  lastUpdated: number;
};

// On AthleteProfile:
surfaceCalibration: Record<SurfaceType, CalibrationStats>;
```

When `sampleCount >= 5` for a surface, `'calibrated'` mode activates automatically for future sprints on that surface — no user action needed.

---

### Surface Type — First-Class Field

Calibration ratios are surface-specific. Surface is set manually by the athlete per session (auto-detection is a future extension):
...
- `PowerProfileSession.surfaceType: SurfaceType | null`
- `SprintPowerRecord.surfaceType: SurfaceType | null` (inherited from session, overridable)
- A ratio learned on gym turf is never applied to outdoor concrete

**Sled type (future):** `sledTypeId: string | null` on sprint records. Sled contact plate area directly affects friction force — different sleds on the same surface may require separate calibration ratios.

---

### CI Peak Zone Density — Mode-Aware Quality Filter

`qualityFilter` is replaced by a `minMode` parameter:
...
calculatePeakZoneDensity(
  ...
  pplEstimate,
  windowDays = 90,
  minMode: PowerMeasurementMode = 'raw'  // raise as measurement quality improves
)
```

| `minMode`               | What counts toward density                       |
| ----------------------- | ------------------------------------------------ |
| `'raw'` (default today) | All sprints near PPL                             |
| `'calibrated'`          | Sprints with at least a surface-ratio correction |
| `'friction_corrected'`  | Sprints with live friction detection             |
| `'fused'`               | Only cross-validated sprints                     |

Raising `minMode` requires no call-site changes — update the default when measurement maturity warrants it.

---

### Transition Path

**Raw → Calibrated (automatic):** Once 5 calibration sprints accumulate for a surface (during sessions where friction detection was partially active), `calibration-service` automatically applies the learned ratio to future sprints on that surface. Existing raw sprints are not re-stamped.

**Calibrated → Friction corrected (at friction validation):** A one-time prompt appears:

> _"Friction detection is now active. Run a quick Peak Power Test to update your profile with friction-corrected data."_

- **Immediate retest** → `'targeted_retest'` mode; 1–2 sprints restore CI quickly
- **Gradual build** → dismiss; new sprints accumulate friction-corrected values naturally

`frictionActivationAcknowledged: boolean` on `AthleteProfile` tracks whether the prompt has been shown.

**Fused (automatic, long-term):** No user action. `power-source-service` silently elevates to `'fused'` when both channels are active, confident, and in agreement.

---

### User-Facing Presentation

Per-sprint mode label (on sprint cards and session summary):

| Mode                   | Display label          |
| ---------------------- | ---------------------- |
| `'raw'`                | Derived power          |
| `'calibrated'`         | Estimated (calibrated) |
| `'friction_corrected'` | Friction corrected     |
| `'fused'`              | High confidence        |

The CI badge communicates overall profile confidence. The per-sprint mode label communicates measurement method. These are complementary.
...
- `power-measurement-mode.ts` — `PowerMeasurementMode = 'raw' | 'calibrated' | 'friction_corrected' | 'fused'`; `SurfaceType` enum; `CalibrationStats` type
- `athlete-profile.ts` — `AthleteProfile` (`bodyWeightKg: number | null`, `frictionActivationAcknowledged: boolean`, `incompleteDiscoverySessionId: string | null`, `historicalPeakPower: number | null`, `historicalPeakPowerLoad: number | null`, `surfaceCalibration: Record<SurfaceType, CalibrationStats>`), `PPLRevision` (includes `pplLoadKg`, `peakPowerW`, `powerMeasurementMode: PowerMeasurementMode`, `pplAsPctBW: number | null`, `ambiguousPeak: boolean`, `timestamp`) — mode is immutable after write; PPL values across different modes are not numerically comparable, `FVClassification` enum, `HistoryDepth` enum
- `sprint-power-record.ts` — extends `WorkoutMetrics` with `athleteId: string | null`, `loadKg`, `peakVelocity: number`, `powerMeasurementMode: PowerMeasurementMode`, `frictionConfidence: 'Unknown' | 'Low' | 'Medium' | 'High'`, `calibrationRatio: number | null`, `surfaceType: SurfaceType | null`, `sledTypeId: string | null`, `pplAtTimeOfSprint`, `zoneAtRecording`, `returnDetectionFlag`
- `power-session.ts` — `PowerProfileSession` (sessionId, `athleteId: string | null`, sprintIds[], sessionPeakPower, sessionPPLEstimate, ciSnapshot, startedAt, completedAt, `testStatus: 'complete' | 'incomplete' | 'not_a_test'`, `testMode: 'discovery' | 'targeted_retest' | null`, `surfaceType: SurfaceType | null`)
...
- `src/features/power-profile/constants.ts` — `MIN_CALIBRATION_SAMPLES = 5` (threshold for activating calibrated mode)

### Services (`services/`)

- `power-source-service.ts` — `resolveSprintPower(metrics, surface, calibration)` tiered selection; returns `{ power, mode: PowerMeasurementMode }`; today only `'raw'` and `'calibrated'` branches are active
- `calibration-service.ts` — tracks `calibrationRatio` per sprint; maintains `CalibrationStats` per `SurfaceType` using Welford's algorithm; `updateCalibration(athleteProfile, sprintRecord)` and `getCalibration(athleteProfile, surface)`
...
- `src/types/workout-database.ts` — add `athleteId?: string | null`, `loadKg?`, `peakVelocity?`, `powerMeasurementMode?`, `frictionConfidence?`, `calibrationRatio?`, `surfaceType?`, `sledTypeId?`, `pplAtTimeOfSprint?`, `zoneAtRecording?`, `sessionId?` to `WorkoutMetrics` / `WorkoutEntry`
...
Shown automatically as a full screen immediately after the athlete taps "End Session". Navigates to `session-summary.screen.tsx`.

#### `PowerProfileSession` — new fields populated at `endSession()`

```ts
pplAtSessionStart: number | null    // PPL locked at session start; drives zone bands on session summary chart
pplAtSessionEnd: number | null      // set only if PPL was updated mid-session (PB)
totalWorkJoules: number             // sum of ∫(m·a)·v dt per sprint — derived from ProcessedSensorData
sessionPeakVelocity: number
averagePeakPower: number
averagePeakVelocity: number
totalDrives: number                 // sum of drive counts across all sprints
avgDrivesPerSprint: number
avgDrivesToPeakPower: number        // avg across sprints: drive index at which peak power occurred
powerTrend: number[]                // per-sprint peak power in order — for trend chart
velocityTrend: number[]             // per-sprint peak velocity in order
fatigueFlag: boolean                // true if power declined >8% from best sprint to final sprint
zoneDistribution: Record<TrainingZone, number>  // count of sprints per zone
```

#### Screen: `screens/session-summary.screen.tsx`
...
| --------------------- | ----------------------------------------------------- |
| Total sprints         | count                                                 |
| Session peak power    | W                                                     |
| Session peak velocity | m/s                                                   |
| Total work            | Joules (physics-based: ∫F·v dt summed across sprints) |

**2. Power profile curve** (`ppl-power-curve` component)

- All session sprints plotted as dots (load on X, power + velocity on their respective axes)
- Zone bands based on `pplAtSessionStart` — frozen to session-start PPL regardless of any mid-session update
- If PPL was updated mid-session: inline note beneath chart — _"Your PPL was updated this session. Zone bands reflect your PPL at session start ([X] kg)."_

**3. Sprint trend chart** (`components/session-trend-chart.tsx`)

- X axis: sprint number (1, 2, 3…)
- Y axis: peak power (W) — line + dots
- Secondary Y axis: peak velocity (m/s) — secondary line
- Text indicator overlaid or below: _"Power held steady across the session"_ / _"Power declined 12% by the final sprint"_ / _"Power built across the session — great session"_
- `fatigueFlag = true` renders the indicator in a warning colour

**4. Zone distribution bar**

- Horizontal segmented bar: each zone's proportion of total sprints, colour-coded by zone
- Label per segment: zone name + count (e.g. "Peak Power · 4 sprints")

**5. Drive summary tiles** (using `Tile` primitive)

- Total drives
- Avg drives per sprint
- Avg drives to peak power

**6. Profile / CI context strip**

- CI badge (post-session value)
- If PPL changed this session: "New PPL: X kg ↑ from Y kg"
- Any overload or return detection signal that fired this session

**7. Sprint list**

- FlashList of `sprint-history-card` components for this session
- Tappable to sprint detail screen
